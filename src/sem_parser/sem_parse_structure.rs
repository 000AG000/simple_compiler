...
impl<T> Spanned<T>{
    pub fn lexeme<'a>(&self,input_str: &'a str)->&'a str{
        debug_assert!((input_str.len()>=self.span.start) | (input_str.len()>=self.span.end),"spanned element out of bounds");
...
impl Statement{
    pub fn pretty_print(&self, input_str:&str) -> String{
        match &self.node {
            ...
            StatementKind::Assign { name, value } => format!("Assign {} = {}",name.lexeme(input_str),value.lexeme(input_str)),
