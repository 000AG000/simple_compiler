...
impl RuntimeContext {
    ...
    pub fn contains_variable(&mut self, ident_id: &IdentId) -> bool {
...
impl<'a> Interpreter<'a> {
    ...
    pub fn interpet_expr(&mut self, expr: &Expr) -> usize {
        match &expr.node {
            ...
            crate::sem_parser::ExprKind::Binary { left, op, right } => {
                let left_expr_eval = self.interpet_expr(&left);
                let right_expr_eval = self.interpet_expr(&right);
